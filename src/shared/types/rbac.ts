...
export interface IRBACProvider {
  ...
  assignRoleToUser(userId: string, roleId: string, options?: AssignRoleOptions): Promise<UserRole>
  removeRoleFromUser(userId: string, roleId: string, organizationId?: string): Promise<void>

  // Permission Checking
  userHasPermission(userId: string, permissionName: string, organizationId?: string): Promise<boolean>
  userHasRole(userId: string, roleName: string, organizationId?: string): Promise<boolean>
...
export type DefaultPermission = typeof DEFAULT_PERMISSIONS[keyof typeof DEFAULT_PERMISSIONS]
...
export interface ResourceAction {
  resource: string
  action: 'create' | 'read' | 'update' | 'delete' | 'invite' | 'publish' | 'billing' | 'export'
}
