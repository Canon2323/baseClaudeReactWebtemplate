...
export interface IPaymentProvider {
  ...
  // Customers
  createCustomer(data: Omit<Customer, 'id' | 'stripeCustomerId'>): Promise<Customer>
  getCustomer(customerId: string): Promise<Customer | null>
  updateCustomer(customerId: string, data: Partial<Customer>): Promise<Customer>

  // Subscriptions
  createSubscription(customerId: string, priceId: string, options?: CreateSubscriptionOptions): Promise<Subscription>
...
export interface PaymentError {
  code: string
  message: string
  type: 'api_error' | 'card_error' | 'idempotency_error' | 'invalid_request_error' | 'rate_limit_error'
  details?: any
}
...
export const ACTIVE_SUBSCRIPTION_STATUSES: Subscription['status'][] = ['active', 'trialing']
export const INACTIVE_SUBSCRIPTION_STATUSES: Subscription['status'][] = ['canceled', 'incomplete', 'incomplete_expired', 'past_due', 'unpaid']
