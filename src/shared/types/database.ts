...
export interface IDatabaseProvider {
  ...
  // Utilidades
  count(table: string, options?: Pick<QueryOptions, 'where'>): Promise<DatabaseResponse<number>>
  exists(table: string, id: string): Promise<DatabaseResponse<boolean>>

  // Storage (se aplicável)
  uploadFile?(bucket: string, path: string, file: File | Buffer): Promise<DatabaseResponse<{ path: string; url: string }>>
  downloadFile?(bucket: string, path: string): Promise<DatabaseResponse<{ data: Blob; url: string }>>
...
export type DatabaseProviderType = 'supabase' | 'planetscale' | 'prisma' | 'mongodb'
