...
import type { IRBACProvider, RBACProviderType, RBACProviderConfig } from '@/shared/types/rbac'

// Factory for creating RBAC providers (Factory Pattern + Strategy Pattern)
export class RBACProviderFactory {
  private static providers: Map<RBACProviderType, () => Promise<IRBACProvider>> = new Map()
  ...
  static async createProvider(config: RBACProviderConfig): Promise<IRBACProvider> {
...
export const registerDefaultRBACProviders = async () => {
  ...
  RBACProviderFactory.registerProvider('supabase', async () => {
    const { SupabaseRBACProvider } = await import('./providers/supabase-rbac-provider')
  ...
  RBACProviderFactory.registerProvider('database', async () => {
    const { DatabaseRBACProvider } = await import('./providers/database-rbac-provider')
  ...
  switch (config.type) {
    ...
    case 'database':
      if (!config.options.connectionString) {
        throw new Error('Database provider requires connectionString in options')
      }
      break
...
export class RBACManager {
  ...
  static getInstance(): IRBACProvider {
    if (!this.instance) {
      throw new Error('RBAC not initialized. Call RBACManager.initialize() first.')
    }
    return this.instance
  }
