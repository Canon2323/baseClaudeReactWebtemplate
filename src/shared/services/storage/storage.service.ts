...
import { StorageError, StorageQuotaError, EncryptionError } from './storage.types'
...
const storageOptionsSchema = z.object({
  provider: z.enum(['localStorage', 'sessionStorage', 'memory', 'cookie']).optional(),
...
export class StorageService implements IStorageService {
  ...
  async set<T>(key: string, value: T, options: StorageOptions = {}): Promise<void> {
    ...
    try {
      ...
      throw new StorageError(`Failed to set storage item: ${error}`, 'SET_FAILED')
    }
  }
  ...
  async get<T>(key: string, defaultValue?: T): Promise<T | null> {
    ...
    try {
      ...
      const providers: StorageProviderType[] = ['localStorage', 'sessionStorage', 'memory']
      ...
      throw new StorageError(`Failed to get storage item: ${error}`, 'GET_FAILED')
    }
  }
  ...
  async remove(key: string): Promise<void> {
    ...
    try {
      ...
      throw new StorageError(`Failed to remove storage item: ${error}`, 'REMOVE_FAILED')
    }
  }
  ...
  async clear(): Promise<void> {
    try {
      ...
      throw new StorageError(`Failed to clear storage: ${error}`, 'CLEAR_FAILED')
    }
  }
  ...
  async keys(): Promise<string[]> {
    try {
      ...
      throw new StorageError(`Failed to get storage keys: ${error}`, 'KEYS_FAILED')
    }
  }
  ...
  async setSecure<T>(key: string, value: T, options: SecureStorageOptions = {}): Promise<void> {
    ...
    try {
      ...
      throw new EncryptionError(`Failed to store secure item: ${error}`, 'encrypt')
    }
  }
  ...
  async getSecure<T>(key: string, defaultValue?: T): Promise<T | null> {
    ...
    try {
      ...
      throw new EncryptionError(`Failed to retrieve secure item: ${error}`, 'decrypt')
    }
  }
  ...
  async getUsage(): Promise<StorageUsage> {
    try {
      ...
      throw new StorageError(`Failed to get storage usage: ${error}`, 'USAGE_FAILED')
    }
  }
  ...
  async cleanup(strategy: CleanupStrategy = { type: 'ttl' }): Promise<void> {
    try {
      ...
      throw new StorageError(`Failed to cleanup storage: ${error}`, 'CLEANUP_FAILED')
    }
  }
  ...
  private async validateCleanupStrategy(strategy: CleanupStrategy): Promise<void> {
    try {
      cleanupStrategySchema.parse(strategy)
    } catch (error) {
      throw new StorageError(`Invalid cleanup strategy: ${error}`, 'INVALID_STRATEGY')
    }
  }
