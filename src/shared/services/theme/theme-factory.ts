import type { CustomTheme, ThemeColors, CreateThemeOptions, ThemePreset } from '@/shared/types/theme'

// Temas base predefinidos (Template Method Pattern)
const baseThemes: Record<ThemePreset, { light: ThemeColors; dark: ThemeColors }> = {
  default: {
    light: {
      background: '0 0% 100%',
...
export class ThemeFactory {
  ...
  static createTheme(id: string, name: string, options: CreateThemeOptions = {}): CustomTheme {
    const { preset = 'default', customColors, customSpacing, customFonts } = options
  ...
  static validateTheme(theme: CustomTheme): boolean {
    const requiredColorKeys = [
      ...
      'secondary', 'secondaryForeground', 'border', 'ring'
    ]

    const hasAllLightColors = requiredColorKeys.every(key =>
      key in theme.colors.light
    )
    const hasAllDarkColors = requiredColorKeys.every(key =>
      key in theme.colors.dark
    )
