...
export function useRBAC(organizationId?: string): UseRBACReturn {
  const { user } = useAuth()
...
export function useHasAnyPermission(permissions: string[], organizationId?: string): boolean {
...
export function useHasAllPermissions(permissions: string[], organizationId?: string): boolean {
  const { hasPermission, loading } = useRBAC(organizationId)
...
export function useCanAccessResource(resource: string, actions: string[] = ['read'], organizationId?: string): {
  canRead: boolean
  canWrite: boolean
  canDelete: boolean
